...
export default function MisionScreen() {
  ...
  const [mision, setMision] = useState('');  // Estado para almacenar la misión
  const [loading, setLoading] = useState(true); // Estado de carga

  useEffect(() => {
    // Solicitar la misión desde la API
    axios.get('http://localhost:8082/api/empresa/misiones') // Cambia esto por la URL de tu API
      .then((response) => {
        const data = response.data as { contenido: string }[]; // Cast response.data to the expected type
        setMision(data[0]?.contenido || 'No se ha definido misión.');
        setLoading(false);
      })
      .catch((error) => {
      ...
      <ScrollView style={{ flex: 1 }}>
        ...
        <View style={styles.cardContainer}>
          ...
          <View style={styles.mainContent}>
            ...
            {/* Mostrar loading mientras se obtiene la misión */}
            {loading ? (
              <Text style={styles.parrafo}>Cargando misión...</Text>
            ) : (
